...
import { useComments, useCreateComment, useDeleteComment, useLikeComment, useUnlikeComment } from '@/hooks/use-comments';
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { Button } from '@/components/ui/button';
import { Textarea } from '@/components/ui/textarea';
import { formatRelativeTime, getInitials, cn, formatCount } from '@/lib/utils';
...
function CommentItem({ comment, postId }: { comment: Comment; postId: string }) {
  ...
  const handleReplySubmit = () => {
    ...
    createComment.mutate({ postId, textContent: replyContent, parentId: comment.id }, {
      onSuccess: () => {
        setIsReplying(false);
        setReplyContent('');
      }
...
export function CommentSection({ postId }: CommentSectionProps) {
  ...
  const handleCreateComment = () => {
    ...
    createComment.mutate({ postId, textContent: content }, {
          ...
          {rootComments.map((comment: Comment) => (
            <div key={comment.id}>
              ...
              <div className="ml-10 mt-2 space-y-2 border-l-2 border-border pl-4">
                {replies.filter((r: Comment) => r.parentId === comment.id).map((reply: Comment) => (
