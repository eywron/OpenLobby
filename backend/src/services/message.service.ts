...
export async function sendMessage(conversationId: string, senderId: string, data: SendMessageInput) {
...
export async function getMessages(conversationId: string, userId: string, query: GetMessagesQuery) {
	...
	const messages = await messageRepository.findMessagesByConversationId(conversationId, query.limit, query.cursor);
